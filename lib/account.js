...
this.accountInfo = async (req, res) => {
  ...
  if (this.lib.isLoggedIn(req)) {
    ...
    if (profileInformation.length >= 1 && profileInformation[0].length >= 1) {
      ...
      res.type('text/json').status(400).send(accountInfoFailure);
    }
  } else {
...
this.modifyAccount = (req, res) => {
  const { body } = req;
  if (this.lib.isLoggedIn(req)) {
    const data = this.lib.accountInfo(req);
    if ('name' in body && (typeof body.name) === (typeof '')) {
      mysql.query('update profile_information set name=? where uuid=?;', [body.name, data.uuid]);
      data.name = body.name;
    }
