...
this.getNotifications = async (req, res) => {
  if (account.isLoggedIn(req)) {
    ...
    for (let i = 0; i < notifications.length; i += 1) {
      messages.push(this.lib.packMessage(notifications[i].message_type, notifications[i].content, notifications[i].cuuid, notifications[i].message_sent, notifications[i].sender));
    }
...
this.lib.packMessage = (messageType, content, cuuid, message_sent, sender) => {
  if (messageType === 0) {
    const message = packedMessage;
    message.message_type = 'text';
    message.content = content;
    message.cuuid = cuuid;
    message.message_sent = message_sent;
