...
this.getNotifications = async (req, res) => {
  if (account.isLoggedIn(req)) {
    ...
    for (let i = 0; i < notifications.length; i += 1) {
      messages.push(this.lib.packMessage(notifications[i].message_type, notifications[i].content, notifications[i].cuuid));
    }
...
this.lib.packMessage = (messageType, content, cuuid) => {
  if (messageType === 0) {
