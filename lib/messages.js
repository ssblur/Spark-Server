...
this.getNotifications = async (req, res) => {
  if (account.isLoggedIn(req)) {
    ...
    for (let i = 0; i < notifications.length; i += 1) {
      messages.push(this.lib.packMessage(notifications[i].message_type, notifications[i].content, notifications[i].cuuid, notifications[i].message_sent));
    }
...
this.lib.packMessage = (messageType, content, cuuid, message_sent) => {
  if (messageType === 0) {
