  ...
  if (account.isLoggedIn(req)) {
    if (body.cuuid) {
      if (await this.lib.isInChat(req)) {
        ...
        res.type('text/json').status(400).send(getUsersUserNotInChat);
      }
    } else {
      res.type('text/json').status(400).send(getUsersNoChatSpecified);
    }
  } else {
    res.type('text/json').status(400).send(getUsersNotLoggedIn);
  }
};
...
this.getNotifications = async (req, res) => {
  if (account.isLoggedIn(req)) {
    ...
    for (let i = 0; i < notifications.length; i += 1) {
      messages.push(this.lib.packMessage(notifications[i].message_type, notifications[i].content));
    }
    getNotificationsSuccess.content.messages = messages;
    res.type('text/json').send(getNotificationsSuccess);
  } else {
    res.type('text/json').status(400).send(getNotificationsNotLoggedIn);
  }
};
