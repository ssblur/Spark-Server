...
this.modifyChat = async (req, res) => {
  ...
  if (account.isLoggedIn(req)) {
    ...
    if (body.cuuid && await this.lib.isInChat(req)) {
      ...
      (async () => {
        const [type, update] = this.lib.unpackMessage(this.lib.packMessage({
          type: 'chat_update',
          content: {
            name: body.name,
        ...
        mysql.query('insert into chat_messages (uuid, message_type, content, message_sent, sender) values (?, ?, ?, ?, ?);', [body.cuuid, type, update, now, accountInfo.uuid]);
