...
this.dispatchGet = (_req, res) => res.status(405).type('text/json').send(loginGet);
...
this.verifyGet = (_req, res) => res.status(405).type('text/json').send(verifyGet);
...
this.modifyGet = (_req, res) => res.status(405).type('text/json').send(modifyGet);
...
this.searchGet = (_req, res) => res.status(405).type('text/json').send(searchGet);
...
this.addMemberGet = (_req, res) => res.status(405).type('text/json').send(addMemberGet);
...
this.modifyChatGet = (_req, res) => res.status(405).type('text/json').send(modifyChatGet);
...
this.chatInfoGet = (_req, res) => res.status(405).type('text/json').send(chatInfoGet);
...
this.chatSendGet = (_req, res) => res.status(405).type('text/json').send(chatSendGet);

// A default for GET requests on the getMessages hook.
const getMessagesGet = {
  status: 405,
  content: {
    comment: 'The get chat messages function does not support GET requests. Please read API documentation prior to submitting requests.',
  },
};

this.getMessagesGet = (_req, res) => res.status(405).type('text/json').send(getMessagesGet);
